...
export const insertAccounts = mutation({
  ...
  handler: async (ctx, { accounts }) => {
    ...
    for (const account of accounts) {
      // Look up user by supabaseId to get the Convex userId
      const user = await ctx.db
        .query("users")
        .withIndex("by_supabaseId", (q) =>
          q.eq("supabaseId", account.supabaseUserId)
        )
        .first();
      const id = await ctx.db.insert("accounts", {
        ...account,
        userId: user?._id ?? account.supabaseUserId,
      });
      results.push(id);
    }
    return { inserted: results.length };
  ...
  handler: async (ctx, { sessions }) => {
    ...
    for (const session of sessions) {
      const user = await ctx.db
        .query("users")
        .withIndex("by_supabaseId", (q) =>
          q.eq("supabaseId", session.supabaseUserId)
        )
        .first();
      const id = await ctx.db.insert("sessions", {
        ...session,
        userId: user?._id ?? session.supabaseUserId,
      });
      results.push(id);
    }
    return { inserted: results.length };
